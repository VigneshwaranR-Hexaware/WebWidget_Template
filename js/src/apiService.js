...
define(['jquery', 'settings', 'utils', 'messageTemplates', 'cards', 'uuid'],
    function ($, config, utils, messageTpl, cards, uuidv1) {
        ...
        //class ApiHandler {
            ...
            // }
           var methods ={};
           methods.userSays = function(userInput, callback) {
            ...
            methods.askBot=function(userInput, userText, callback) {
              options.query = userInput;
                console.log(options);
                $("#btn-input").focus();
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response.result.fulfillment.messages) {   
                            if(response.result.action== "OtherActivity" || response.result.action=="EscalationQuery" ){
                                window.location.href='https://server.iad.liveperson.net/hc/70994705/?cmd=file&file=visitorWantsToChat&site=70994705&byhref=1&SESSIONVAR!skill=MyRicohSupport&imageUrl=https://server.iad.liveperson.net/hcp/Gallery/ChatButton-Gallery/English/General/1a'; 'height=400,width=600';
                            }
                        ...
                        if (isCardorCarousel) {
                            if (count == 1) {
                                ...
                                let carouselHTML = cards.getHTML({
                                    "action":response.result.action,
                                    "payload": response.result.fulfillment.messages,
                                    "senderName": config.botTitle,
                                    "senderAvatar": config.botAvatar,
                        ...
                        if (isAirlineBoardingPass) {
                            let boardingPassHTML = cards.getHTML({
                                "payload": response.result.fulfillment.messages,
                                "senderName": config.botTitle,
                                "senderAvatar": config.botAvatar,
                                "time": utils.currentTime(),
