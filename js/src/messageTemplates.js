...
define(["utils","settings"], function (utils,settings) {
    ...
    //User Plain Text
    methods.userplaintext =function(data){
    ...
    //Plain Text Template
    methods.plaintext =function(data){
        let html = '<li class="list-group-item background-color-custom">'+
    ...
    methods.card = function(data){
        ...
        for (let i in data.payload) 
        {
            ...
            if(data.payload[i].title !="" ||data.payload[i].title != undefined ){
                console.log("Title="+data.payload[i].title);
    ...
    methods.quickreplies =function(data){
    ...
    methods.carousel = function (data, uniqueId) {
        ...
        for (let i in data.payload) {
            if (data.payload[i].type == 1) {
              if(data.action == "InvoiceExistsIntent.InvoiceExistsIntent-yes.InvoiceInFront-no"){
	    ...
	    '</div>'+'</div>'+'<p style="bottom: 10px;" class="bot-res-timestamp-card">'+'<small>'+'<img style="border-radius:50%;border:2px solid white;" width="20" height="20" src='+settings.botAvatar+'/>'+data.time+'</small>'+'</p>'+'</div>'+'</li>';
